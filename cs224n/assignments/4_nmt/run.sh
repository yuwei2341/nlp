...
elif [ "$1" = "train_tiny" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --train-src=./en_es_data/train.es --train-tgt=./en_es_data/train.en --dev-src=./en_es_data/dev.es --dev-tgt=./en_es_data/dev.en --max-epoch=10 --vocab=vocab.json --cuda
elif [ "$1" = "test" ]; then
...
elif [ "$1" = "test_tiny" ]; then
        CUDA_VISIBLE_DEVICES=0 python run.py decode model.bin ./en_es_data/test.es.tiny ./en_es_data/test.en.tiny outputs/test_outputs.tiny.txt --cuda
