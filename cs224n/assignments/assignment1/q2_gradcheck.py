...
def gradcheck_naive(f, x):
    ...
    while not it.finished:
        ...
        numgrad = (f(x_up)[0] - f(x_down)[0]) / (2 * h)
        ### END YOUR CODE
...
def your_sanity_checks():
    ...
    ### YOUR CODE HERE
    from sigmoid_sum import sigmoid_sum, sigmoid_sum_grad
    quad = lambda x: (sigmoid_sum(x), sigmoid_sum_grad(x))

    print "Running sanity checks..."
    # gradcheck_naive(quad, np.array(1.0))      # scalar test
    # gradcheck_naive(quad, np.random.randn(3,))    # 1-D test
    gradcheck_naive(quad, np.random.randn(4,5))   # 2-D test
